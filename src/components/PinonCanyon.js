import { pinonData } from '../data/pinonData.js';
import { resolvePath } from '../utils/paths.js';

export const PinonCanyon = (target) => {
  const sectionsHtml = pinonData.sections.map((section, index) => `
    <div class="pinon-section" style="
      display: flex;
  ...
  const content = `
    <div style="background-color: var(--color-paper); min-height: 100vh;">
      <!-- Hero Header -->
      <div style="
        background: linear-gradient(rgba(0,0,0,0.4), rgba(0,0,0,0.4)), url('${resolvePath('/assets/images/pinon_canyon.png')}');
      ...
      ">
        <div class="container" style="background: rgba(0,0,0,0.6); padding: 2rem; border-radius: 8px; backdrop-filter: blur(4px);">
          <h1 style="font-size: 3.5rem; margin-bottom: var(--spacing-md); color: var(--color-sand);">${pinonData.title}</h1>
          <p style="font-size: 1.5rem; font-family: var(--font-heading); color: white;">${pinonData.subtitle}</p>
        </div>
      </div>

      <div class="container" style="max-width: 1000px;">
        <div class="intro mb-lg text-center">
          <p style="font-size: 1.3rem; font-weight: 500; color: var(--color-charcoal);">${pinonData.intro}</p>
        ...
        <div class="text-center" style="margin-top: 4rem; padding-bottom: 4rem;">
          <h3 style="margin-bottom: var(--spacing-md);">A Community United</h3>
          <p class="mb-lg">The victory against expansion showed the power of the people.</p>
